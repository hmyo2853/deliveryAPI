...
- `CSS` 전처리기 `SASS` 사용
- `React-Query`로 비동기 프로그래밍 상태 관리
- 상태관리 `Redux` 공부 및 사용
- `vite create` 및 `vercel` 배포
- CSS library `Material UI` 사용
- `firebase`로 API Key 숨기기
...
- [ ] `React-Query`로 API data 상태 관리, 로딩/에러 처리
- [ ] 스마트택배 API 연동
- [ ] 기존 state 구현 완료 후, `Redux`로 리팩토링
- [ ] `firebase`로 API Key 관리
- [ ] 웹, 앱 반응형 UI 구현 (모바일 고려)
